	...
	<table width="100%" color="#999" cellpadding="0" cellspacing="0" border="1" style="border-color:#eee"  >
		...
		<form action="updateOk" method="get" name="update_frm">
			<tr height="30">
				...
				<select id="factnm" name="factnm" value="${rdto.factnm}">
				  <option value="">--선택--</option>
					<c:forEach items="${nfact_list}" var="ndto">
	    				<option value="${ndto.factnm}" <c:if test="${ndto.factnm == rdto.factnm}">selected</c:if>>${ndto.factnm}</option>
	    			</c:forEach>
				</select>
				</td>
				...
				<td align="center"><textarea id="textarea01" rows="5" cols="24" name="requestcomment" >${rdto.requestcomment}</textarea></td>
				<td align="center">
				<select id="managernm" name="managernm" value="${mdto.managernm}">
                  <option value="">--선택--</option>
